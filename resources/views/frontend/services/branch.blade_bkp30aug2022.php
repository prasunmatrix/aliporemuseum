    ...
    $.ajax({
      ...
      success: function(res) {
        ...
        for (var i = 0; i < res.list.length; i++) {
          ...
          list += '<a  href="#" onclick="showMap(' + res.list[i].latitude + ', ' + res.list[i].longitude + ')" data-toggle="modal" data-target="#myModal" class="customcard-link trigger">Show Map</a>';
          list += '</div>';
          list += '</div>';
        }
